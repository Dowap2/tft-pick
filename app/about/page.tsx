...
export default async function AboutPage() {
  ...
  return (
    <main className="w-full max-w-3xl px-4 py-8 sm:py-10">
      ...
      <H>데이터 출처</H>
      <P>
        덱과 통계는 <strong>Riot Games 공식 API(TFT Match-V5)</strong>로 받은 한국 서버 챌린저·그랜드마스터 랭크 게임에서 직접 만듭니다.
        현재 세트 {meta.set}(패치 {patch === "json" ? meta.patch : patch}), 최근 7일, 덱에 배정된 보드 <span className="num">{games.toLocaleString()}</span>개 기준입니다.
      ...
      <P>
        ...
        캐리는 <em>공격 아이템을 가장 많이 드는 유닛</em>, 아이템 우선순위는 그 유닛의 아이템 등장률 순, 이름은 가장 많이 활성화된 2단계 이상 시너지 + 캐리입니다.
      </P>

      <H>티어(OP·S·A·B)</H>
      <P>
        30판 이상, 평균 4.75등 이내인 덱만 대상으로 합니다. 승률과 평균 등수가 모두 상위 10%면 <strong>OP</strong>,
        그 다음 (승률 순위 + 등수 순위) 상위 5개가 <strong>S</strong>, 나머지를 4:6으로 나눠 <strong>A</strong>·<strong>B</strong>입니다.
        표본이 적을 때는 티어가 흔들릴 수 있어 카드에 게임 수를 함께 표시합니다.
      </P>
      ...
      <ul className="list-disc space-y-1 pl-5 text-sm leading-6 text-text/90">
        ...
        <li>표본이 하루 단위로 쌓이는 초기라 티어와 순위는 매일 바뀔 수 있습니다.</li>
      </ul>

      <H>갱신 주기</H>
      <P>매일 오전 6시(KST)에 새 게임을 수집하고 덱과 통계를 다시 계산합니다. 페이지 하단에 기준 패치가 표시됩니다.</P>
