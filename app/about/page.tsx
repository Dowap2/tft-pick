...
export default async function AboutPage() {
  ...
  return (
    <main className="w-full max-w-3xl px-4 py-8 sm:py-10">
      ...
      <P>
        자격은 <strong>최근 7일 100판 이상, 평균 4.75등 이내</strong>. 그중 승률과 평균 등수가 모두 상위 10%면 <strong>OP</strong>,
        그 다음 (승률 순위 + 등수 순위) 상위 5개가 <strong>S</strong>, 나머지를 <strong>A</strong> 30% · <strong>B</strong> 40% · <strong>C</strong> 30%로 나눕니다.
        표본 하한을 100판으로 두는 건, 수십 판짜리 덱이 수천 판 검증된 덱 위에 올라오는 걸 막기 위해서입니다. <strong>OP가 없는 날도 정상입니다.</strong>
      ...
      <ul className="list-disc space-y-1 pl-5 text-sm leading-6 text-text/90">
        ...
        <li>덱 상성은 &quot;같은 로비에 있을 때 평균 등수 변화&quot;입니다. 전투에서 지는 것과 같은 기물을 두고 경합해서 둘 다 약해지는 것을 구분하지 못합니다.</li>
        <li>증강은 현재 세트의 Riot API 응답에 포함되지 않습니다. 자체 집계 로직은 준비돼 있지만 데이터가 없어, 추천 증강은 MetaTFT 큐레이션을 씁니다.</li>
